export default function Contacto() {
  return (
    <section id="contacto" className="bg-dark-card section-padding border-t border-dark-border">
      <div className="max-w-7xl mx-auto text-center">

        {/* Badge */}
        <p className="text-gold text-xs tracking-[4px] uppercase mb-5">
          Contáctanos
        </p>

        {/* Título */}
        <h2 className="text-white font-black uppercase leading-none text-4xl md:text-5xl lg:text-6xl mb-5">
          ¿Listo para Elevar<br />tu Imagen?
        </h2>

        {/* Subtítulo */}
        <p className="text-gray-400 text-sm max-w-xl mx-auto leading-relaxed mb-12">
          Reserva tu espacio con Lobo y descubre por qué Visionary Studio es
          la referencia del estilo en Liberia, Guanacaste.
        </p>

        {/* Fila de acción */}
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-10 mb-12">

          {/* WhatsApp */}
          <a
            href="https://wa.me/50662009558"
            target="_blank"
            className="flex items-center gap-3 text-white hover:text-gold transition-colors duration-300"
          >
            <svg className="w-5 h-5 text-gold shrink-0" fill="currentColor" viewBox="0 0 24 24">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347zM12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.132 1.528 5.881L0 24l6.266-1.641A11.945 11.945 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.818 9.818 0 01-5.006-1.369l-.36-.214-3.722.976.993-3.622-.235-.373A9.818 9.818 0 0112 2.182c5.42 0 9.818 4.398 9.818 9.818 0 5.421-4.398 9.818-9.818 9.818z" />
            </svg>
            <span className="font-bold tracking-wide">+506 6200-9558</span>
          </a>

          {/* Botón CTA */}
          <a
            href="/reservar"
            className="btn-gold uppercase tracking-widest text-sm px-10 py-4"
          >
            Reservar Ahora
          </a>

          {/* Instagram */}
          <a
            href="https://www.instagram.com/lobo_barbero"
            target="_blank"
            className="flex items-center gap-3 text-white hover:text-gold transition-colors duration-300"
          >
            <svg className="w-5 h-5 text-gold shrink-0" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
            </svg>
            <span className="font-bold tracking-wide">@lobo_barbero</span>
          </a>

        </div>

        {/* Horarios y ubicación */}
        <div className="flex flex-wrap justify-center gap-x-8 gap-y-2 text-gray-500 text-xs tracking-wider">
          <span>Lunes a Sábado: 8:30am – 6:00pm</span>
          <span className="hidden sm:inline text-dark-border">|</span>
          <span>Domingo: Cerrado</span>
          <span className="hidden sm:inline text-dark-border">|</span>
          <a href="https://maps.app.goo.gl/zcfrCQAJDv4KLDfb9" target="_blank" className="hover:text-gold transition-colors duration-300">
  📍 Liberia, Guanacaste, Costa Rica
</a>
        </div>

      </div>
    </section>
  );
}
